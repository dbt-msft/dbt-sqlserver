...
- Fix a `view` model whose SQL text is unchanged (and so skips its `CREATE`/`ALTER`) going stale when something it reads changes shape - a referenced table or view gaining, losing or reordering columns, or changing a column's type. SQL Server derives a view's column metadata, including the expansion of an unqualified `select *`, at `CREATE VIEW` time and caches it, so skipping that statement left the view reporting the old names, positions and types while every dbt run reported success. The skip path now compares that cached metadata against what the view's body resolves to and runs `sp_refreshview` only where the two disagree, re-deriving it without reissuing the `CREATE`. The comparison rides along with the definition lookup the skip decision already makes, so an unchanged view still costs one round trip and one whose sources have not moved is left untouched - `sys.objects.modify_date` still tells a skip from a rebuild. A shape that cannot be determined is refreshed rather than assumed current, and the refresh carries the model's database as a `USE` prefix so a cross-database view model resolves.
