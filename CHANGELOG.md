...
- Fix `CREATE SCHEMA` racing itself when more than one session builds the same schema at once, failing the run with `There is already an object named '<schema>' in the database` (`Msg 2714`). The four places that create a schema on demand - `sqlserver__create_schema`, `sqlserver__create_schema_with_authorization`, and the copies of the guard carried by `sqlserver__get_test_sql` and `sqlserver__get_unit_test_sql` - each ran `IF NOT EXISTS (SELECT * FROM sys.schemas ...) BEGIN CREATE SCHEMA ... END`, which is check-then-act: with `threads > 1`, or two dbt processes pointed at one database, several sessions pass the check together and all but one fail their create. The run then dies on a schema that, by the time the error is raised, exists and is perfectly usable - which is why a rerun succeeds, and why CI that builds a schema per pull request saw it only on a branch's first run. SQL Server has no `CREATE SCHEMA IF NOT EXISTS`, and catching `2714` would be worse than the error it hides: every connection runs `SET XACT_ABORT ON` (#718), under which the failed create dooms the enclosing transaction - `XACT_STATE()` returns -1 and the transaction is already rolled back by the time the batch reaches its `COMMIT` - so a `TRY`/`CATCH` would convert a loud failure into a silently discarded transaction. All four sites now share one `create_schema_if_not_exists` macro that serializes the check and the create behind a database-scoped `sp_getapplock`, held for the microseconds the create takes and released in the same batch. The lock is session-scoped rather than transaction-scoped because these callers run both inside dbt's transaction and in autocommit; a lock request that times out falls back to the bare check, which is no worse than the previous behaviour. Schema and authorization names are also escaped where they are interpolated into string literals, so a name containing an apostrophe no longer breaks the statement. [#839](https://github.com/dbt-msft/dbt-sqlserver/issues/839)
