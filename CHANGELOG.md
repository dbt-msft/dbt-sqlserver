...
- Fix concurrent `CREATE SCHEMA` failing a run with `There is already an object named '<schema>' in the database` (`Msg 2714`). The four macros that create a schema on demand - `sqlserver__create_schema`, `sqlserver__create_schema_with_authorization`, `sqlserver__get_test_sql` and `sqlserver__get_unit_test_sql` - each guarded it with `IF NOT EXISTS (...) BEGIN CREATE SCHEMA ... END`, which is check-then-act: with `threads > 1`, or two dbt runs against one database, sessions pass the check together and all but one fail. This is why a rerun succeeds, and why CI that builds a schema per pull request only saw it on a branch's first run. All four now share one `create_schema_if_not_exists` macro that serializes the check and the create behind an `sp_getapplock`, released even when the create fails. Schema and authorization names are also escaped where they are interpolated into string literals, so a name containing an apostrophe no longer breaks the statement. [#839](https://github.com/dbt-msft/dbt-sqlserver/issues/839)
