...
- Fix `as_columnstore` CCI naming so creates and rebuilds no longer keep `__dbt_tmp` in the persisted CCI name after swap, which left a temporary-looking index name on the final table. `sqlserver__create_clustered_columnstore_index` now uses the logical/final relation for the index name and the physical relation for `CREATE INDEX`, and `sqlserver__get_create_table_load_sql` now takes `logical_relation` from all callers (`table`, `incremental`, `snapshot`). Legacy `__dbt_tmp_cci` names are dropped before create (no forced `--full-refresh` on upgrade); custom macros calling either function should now pass both relations, however it will work anyway. This addresses the CCI naming slice of [#578](https://github.com/dbt-msft/dbt-sqlserver/issues/578), not the entire issue.
