...
Coverage:
  * relation rendering            - qualified relation quoting
  * sqlserver__strip_dbt_suffix  - __dbt_tmp / __dbt_backup stripping
  * sqlserver__index_name         - determinism, 116-char cap, hashing,
...
class TestStripDbtSuffix:
    ...
    def test_strip(self, identifier, expected):
        assert _render(f"sqlserver__strip_dbt_suffix('{identifier}')") == expected


class TestIndexName:
    def test_deterministic(self):
        rel = _FakeRelation("d", "s", "t")
        a = _render("sqlserver__index_name(rel, 'nidx', ['x','y'])", rel=rel)
        b = _render("sqlserver__index_name(rel, 'nidx', ['x','y'])", rel=rel)
        assert a == b

    def test_column_order_changes_name(self):
        rel = _FakeRelation("d", "s", "t")
        a = _render("sqlserver__index_name(rel, 'nidx', ['x','y'])", rel=rel)
        b = _render("sqlserver__index_name(rel, 'nidx', ['y','x'])", rel=rel)
        assert a != b

    def test_unique_flag_changes_name(self):
        rel = _FakeRelation("d", "s", "t")
        a = _render("sqlserver__index_name(rel, 'cidx', ['x'], unique=False)", rel=rel)
        b = _render("sqlserver__index_name(rel, 'cidx', ['x'], unique=True)", rel=rel)
        assert a != b

    def test_includes_changes_name(self):
        rel = _FakeRelation("d", "s", "t")
        a = _render("sqlserver__index_name(rel, 'nidx', ['x'])", rel=rel)
        b = _render("sqlserver__index_name(rel, 'nidx', ['x'], includes=['y'])", rel=rel)
        assert a != b

    def test_accepts_string_columns(self):
        rel = _FakeRelation("d", "s", "t")
        name = _render("sqlserver__index_name(rel, 'nidx', 'x')", rel=rel)
        assert name.startswith("nidx_t_")

    def test_false_includes_are_treated_as_empty(self):
        rel = _FakeRelation("d", "s", "t")
        a = _render("sqlserver__index_name(rel, 'nidx', ['x'], includes=false)", rel=rel)
        b = _render("sqlserver__index_name(rel, 'nidx', ['x'])", rel=rel)
        assert a == b

    def test_dbt_tmp_suffix_does_not_affect_name(self):
        a = _render(
            "sqlserver__index_name(rel, 'cci', ['__all__'])",
    ...
    def test_length_capped_at_116_chars(self):
        ...
        name = _render("sqlserver__index_name(rel, 'nidx', ['c'])", rel=rel)
        assert len(name) <= 116

    def test_long_name_still_unique_per_signature(self):
        rel = _FakeRelation("d", "s", "x" * 500)
        a = _render("sqlserver__index_name(rel, 'nidx', ['c1'])", rel=rel)
        b = _render("sqlserver__index_name(rel, 'nidx', ['c2'])", rel=rel)
        assert a != b
        assert len(a) <= 116
        assert len(b) <= 116
...
class TestCreateNonclusteredIndex:
    ...
    def test_columns_with_brackets_are_quoted(self):
        rel = _FakeRelation("d", "s", "t")
        sql = _normalize_ws(
